...
Simple bot made for testing out haskell. No multithreading or multicore. Can work with Telegram or VKontakte one at a time. Communication with the social networks is carried out through Long Polling.
...
- Module [Web.Parsing](https://github.com/KonstantinZsky/telegram-bot-on-haskell/blob/master/Lib/Web/Parsing.hs) contains functions that convert the responses of a social network into types that the main bot logic can work with. The answer for the social network is also prepared here. Functions are abstracted from IO and a specific social network by using classes. Some classes are parameterized by types VKSupportData and TelegramSupportData. They are specified in the module [Web.Types](https://github.com/KonstantinZsky/telegram-bot-on-haskell/blob/master/Lib/Web/Types.hs).
